...
export default {
  ...
  methods: {
    ...
    submitComment(index, commentId) {
      ...
      param.text = content;
      axios
        .post(this.$api.commentUpdate, param)
        .then(res => {
          this.loading = false;
          if (res.code == "200") {
    ...
    submitReply(index, commentId) {
      ...
      this.loading = true;
      axios
        .post(this.$api.commentReply, { id: commentId, reply: content })
        .then(res => {
          this.loading = false;
          if (res.code == "200") {
            let mes =
    ...
    clickRecommend(index, bool) {
      ...
      axios
        .post(this.$api.commentRecommend, { id: commentId, isRecommend: bool })
        .then(res => {
          this.loading = false;
          if (res.code == "200") {
    ...
    aduit(ids, status) {
      this.loading = true;
      axios
        .post(this.$api.commentCheck, { ids: ids, isCheck: status })
        .then(res => {
          this.loading = false;
          if (res.code == "200") {
            this.successMessage("操作成功");
    ...
    getTableData() {
      this.loading = true;
      axios
        .post(this.$api.commentItemList, this.params)
        .then(res => {
          this.loading = false;
          if (res.code == "200") {
            this.tableData = [];
...
.header .el-row {
  border-top: 1px #e7ecf3 solid； border-top;
}
.header .title {
  overflow: hidden;
  min-width: 300px;
