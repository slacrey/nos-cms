                  ...
                  <div class="pull-left">
                    <div class="cms-channels" v-for="(item,index) in siteList" :key="index">
                        <div class="pull-left" style="margin-right:12px;">
                          ...
                          <el-input v-model="steps[index]" @blur="inputDigit(steps[index],index)"  ref="reviewLevel" type="number" :disabled="siteIds.indexOf(item.id)=='-1'" class="w50"></el-input>
                          </div>  
  ...
  methods: {
    ...
    getDataInfo(id, https) {
      ...
      axios
        ...
        this.$refs.reviewLevel[index]._data.currentValue = result;
       ...
       for(let x in this.siteIds){
        for(let y in this.siteList){
            if(this.siteList[y].id===this.siteIds[x]){
                let obj={siteIds:this.siteIds[x],steps:this.steps[y],allChannels:this.allChannels[y],allControlChannels:this.allControlChannels[y]}
                source.push(obj);
            }
        }
      }
