package com.nos.cms.staticpage;
...
import com.nos.cms.manager.assist.CmsKeywordMng;
import com.nos.common.hibernate3.Finder;
import com.nos.common.hibernate3.HibernateSimpleDao;
import com.nos.common.page.Paginable;
...
import com.nos.core.web.util.URLHelper;
...
public class StaticPageDaoImpl extends HibernateSimpleDao implements
    ...
    public int channelStatic(Integer siteId, Integer channelId,
        ...
        return handleAtom(conf, data, session, channels);
    }

    private int handleAtom(Configuration conf, Map<String, Object> data, Session session,
                           ScrollableResults channels) throws IOException, TemplateException {
        Writer out = null;
        int count = 0;
        Channel c;
        CmsSite site;
        ...
        while (channels.next()) {
            ...
            totalPage = getTotalPage(c);

            for (int i = 1; i <= totalPage; i++) {
                filename = c.getStaticFilename(i);
                real = realPathResolver.get(filename);
                f = new File(real);
    ...
    private int getTotalPage(Channel c) {
        int totalPage;
        int quantity;// 没有内容或者有子栏目，则只生成一页
