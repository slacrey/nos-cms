package com.nos.cms.action.member;


import com.nos.common.file.FileNameUtils;
import com.nos.common.image.ImageScale;
import com.nos.common.image.ImageUtils;
import com.nos.common.ueditor.LocalizedMessages;
import com.nos.common.ueditor.ResourceType;
import com.nos.common.ueditor.Utils;
...
public class UeditorAct {
	...
	public void imageManager(Integer picNum,Boolean insite,HttpServletRequest request,
		...
		String imgStr;
		StringBuilder imgStrBuff=new StringBuilder();
		...
		for(int i=0;i<picNum;i++){
			...
			imgStrBuff.append(file.getPath().replace(realPathResolver.get(""), site.getContextPath())).append(UE_SEPARATE_UE);
		}
	...
	public void getMovie(HttpServletRequest request,
        ...
        try {
			...
			URL url = new URL(
					"http://api.tudou.com/v6/video/search?app_key=1149d456ca732c4a&format=json&kw="
							+ searchkey + "&pageNo=1&pageSize=20&orderBy=createTime");
...
//        HttpClient client = new InternalHttpClient();// new DefaultHttpClient();
		String outFileName="";
		try{
